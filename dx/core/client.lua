...
function destroy()
	Elements={}
	for i,v in pairs(createdElements) do
...
['Window']=function(v)
			...
			dxDrawText ( v.text, v.x+(v.width*6.5/100), v.y, v.x+v.width-(v.width*6.5/100), v.y+(v.height*5/100), v.titlecolor, v.sizeoftext, "default-bold" ,'center','center',true,false,false)
end,
['Button']=function(v)
				...
				v.isincreas=false
					...
					dxDrawButton(v.x+v.buttoneffectx,v.y+v.buttoneffecty,v.width-(v.buttoneffectx*2),v.height-(v.buttoneffecty*2),v.backcolor2,v.bordercolor2,false)
					dxDrawText ( v.text, v.x, v.y, v.x+v.width, v.y+v.height, v.textcolor2, v.textsize, "default-bold" ,'center','center',true,false,false)
				...
				v.buttoneffecty=0
					...
					dxDrawButton(v.x+v.buttoneffectx,v.y+v.buttoneffecty,v.width-(v.buttoneffectx*2),v.height-(v.buttoneffecty*2),v.backcolor,v.bordercolor,false)
					dxDrawText ( v.text, v.x, v.y, v.x+v.width, v.y+v.height, v.textcolor, v.textsize, "default-bold" ,'center','center',true,false,false)
...
local widthof=(dxGetTextWidth(v.text,1.15,'default-bold')/2)
					...
					dxDrawRectangle(v.x,v.y,v.width,v.height,v.backcolor,false)
					
					if v.distanc2>0 then
					dxDrawLine(v.x,v.y+v.height,v.x+v.distanc2-1,v.y+v.height,v.bordercolor,1,false)
					end
	...
	if v.isenabled then
		...
		if v.x+3+mywidth< v.x+v.width then
			dxDrawText("|",v.x+1+mywidth, v.y, v.x+dxGetTextWidth( '|',1.15,"default-bold")+1, v.y+v.height, tocolor(255,0,0,255), 1.15, "default-bold", "left", "center", false, false, false)
		else
			dxDrawText("|", v.x+v.width-dxGetTextWidth( '|',1.15,"default-bold")-2, v.y, v.x+v.width+dxGetTextWidth( '|',1.15,"default-bold")+2, v.y+v.height, tocolor(255,0,0,255), 1.15, "default-bold", "left", "center", false, false, false)
		end
	end
					
		dxDrawText ( v.text, v.x+v.distanc1-widthof, v.y, v.x+v.width-1, v.y+v.height, v.orignalcolor, 1.15, "default-bold" ,v.position,'center',true,false,false)
...
['Label']=function(v)
					...
					dxDrawText ( v.text, v.x, v.y, v.x+v.width, v.y+v.height, v.textcolor, 1.15, "default-bold" ,v.positionh,v.positiony,true,true,false)
				...
				--------------real deal
					...
					for j,m in ipairs(v.columns)do
						...
						if m.x+m.width>v.x+v.width then
							...
							dxDrawRectangle(m.x,m.y,(v.x+v.width)-m.x,m.height,m.backcolor,false)
							
							dxDrawText ( m.title,m.x,m.y,(v.x+v.width),m.y+m.height, m.color, 1.15, "default-bold" ,'center','center',true,false,false)
					
							dxDrawLine(m.x,(m.y+m.height),(v.width+v.x),(m.y+m.height),v.linecolor,3,false)
					
						else
														
							dxDrawRectangle(m.x,m.y,m.width,m.height,m.backcolor,false)

							dxDrawText ( getMaxStringForWidth(m.title,m.width,false,1.15,'default-bold') ,m.x,m.y,(m.x+m.width),(m.y+m.height), m.color, 1.15, "default-bold" ,'center','center',true,false,false)
														
							dxDrawLine(m.x,(m.y+m.height),(m.x+m.width),(m.y+m.height),v.linecolor,3,false)
					...
					for l,s in ipairs(v.visiblerows)do
						...
						if s[k].iswork then
							...
							if s[k].x and s[k].y then
								...
								dxDrawRectangle(s[k].x,s[k].y,s[k].width,s[k].height,s[k].backcolor,false)
								
									local nstr,old=getMaxStringForWidth(s[k].text,s[k].width,false,1.15,'default-bold')
									
										if old~=nstr then
										
										dxDrawText (  deleteLastCharacter(nstr),s[k].x,s[k].y,(s[k].x+s[k].width),(s[k].y+s[k].height), s[k].color,s[k].scale,s[k].font ,s[k].positiontext,'center',true,false,false)
										
										else
											dxDrawText ( nstr,s[k].x,s[k].y,(s[k].x+s[k].width),(s[k].y+s[k].height), s[k].color,s[k].scale,s[k].font ,s[k].positiontext,'center',true,false,false)
										end
							end
...
['Image']=function(v)
	...
	if v.src then
		...
		dxDrawImage (v.x,v.y,v.width,v.height,v.src,0,0,0,v.backcolor,false)
...
addEventHandler('onClientRender',root,function()
	...
	local x,y=cx/1366,cy/768
	--dxDrawImage(0,0,sx,sy,rt)
	local sx,sy,sz=getWorldFromScreenPosition(cx/2,0,2)
	local ex,ey,ez=getWorldFromScreenPosition(cx/2,cy,2)
	local fx,fy,fz=getWorldFromScreenPosition(cx/2,cy/2,1)
	if isCursorShowing() then
		local mx,my=getCursorPosition()
		mx,my=cx*mx,cy*my
		local fx,fy,fz=getWorldFromScreenPosition(mx,my,1)
		setCursorAlpha(255)
	end
	dxDrawMaterialLine3D(sx,sy,sz,ex,ey,ez,rt,3,tocolor(255,255,255,255),true,fx,fy,fz)]]
end)
