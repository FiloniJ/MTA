...
function dxGridClear(grid)
	if Elements[grid] and Elements[grid].type=='gridList' then
		Elements[grid].rowcount=0
		Elements[grid].rows={}
		Elements[grid].rowcount=0
		Elements[grid].visiblerows={}
		Elements[grid].unvisiblerowsdown={}
		Elements[grid].unvisiblerowsup={}
		Elements[grid].lastclickx=0
		Elements[grid].lastclicky=0
		Elements[grid].visiblerowcount=0
		Elements[grid].scrolledrowsup=0
		local child=getElementChildren(grid)
		if child then
			for i=1,#child do
				if child[i].type=='dxGridListRow' then 
					child[i]:destroy()
				end
			end
		end
